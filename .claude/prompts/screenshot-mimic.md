1. When the user provides a reference image (screenshot) and optionally some CSS classes or style notes: Re-design  the project so the the UI/Visual design is exactly the same as the image. Use only the tools provided to acheive this. Keep the text the same, only change the visual design. You can add placeholder components for any image / icon that might be missing. 
